...
var logger = require('./logger')(logSystem, threadId);
...
setInterval(function(){
    ...
    for (var minerId in connectedMiners){
        ...
        if (now - miner.lastBeat > timeout){
            logger.warn('Miner timed out and disconnected %s@%s', [miner.login, miner.ip]);
    ...
    if (banningEnabled){
        for (ip in bannedIPs){
            ...
            if (now - banTime > config.poolServer.banning.time * 1000) {
                ...
                logger.info('Ban dropped for %s', [ip]);
            }
        }
    }
...
function jobRefresh(loop) {
    return getBlockTemplate().then(result => {
        ...
        logger.error('Error polling getblocktemplate %j', [error]);
        return false;
    }).tap( () => {
        if (loop)
            setTimeout(function(){
...
(function init(){
    jobRefresh(true).then(successful => {
        if (!successful){
            logger.error('Could not start pool');
            return;
        }
...
Miner.prototype = {
    ...
    setNewDiff: function(newDiff){
        ...
        logger.info('Retargetting difficulty %d to %d for %s', [this.difficulty, newDiff, this.login]);
    ...
    checkBan: function(validShare){
        ...
        if (stats.validShares + stats.invalidShares >= config.poolServer.banning.checkThreshold){
            if (stats.invalidShares / stats.validShares >= config.poolServer.banning.invalidPercent / 100){
                logger.warn('Banned %s@%s', [this.login, this.ip]);
    ...
    if (config.poolServer.slushMining.enabled) {                
        if (lastChecked + config.poolServer.slushMining.lastBlockCheckRate <= dateNowSeconds || lastChecked == 0) {
            redisClient.hget(config.coin + ':stats', 'lastBlockFound').then( result => {
                ...
                logger.error('Unable to determine the timestamp of the last block found');
            });
        }
        job.score = job.difficulty * Math.pow(Math.E, ((dateNowSeconds - scoreTime) / config.poolServer.slushMining.weight)); //Score Calculation
        logger.info('Submitted score ' + job.score + ' with difficulty ' + job.difficulty + ' and the time ' + scoreTime);
    } else {
    ...
    redisClient.multi(redisCommands).exec().then(redisClient.convertMultiReply).then(replies => {
        if (blockCandidate){
            ...
            return redisClient.zadd(config.coin + ':blocks:candidates', job.height, [
                ...
                logger.error('Failed inserting block candidate %s \n %j', [hashHex, err]);
            });
        }
    }).catch( error => {
        logger.error('Failed to insert share data into redis %j \n %j', [err, redisCommands]);
    });

    logger.info('Accepted %s share at difficulty %d/%d from %s@%s', [shareType, job.difficulty, shareDiff, miner.login, miner.ip]);
...
function processShare(miner, job, blockTemplate, nonce, resultHash) {
    ...
    if (hash.toString('hex') !== resultHash) {
        logger('warn', 'Bad hash from miner %s@%s', [miner.login, miner.ip]);
        return false;
    }
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')]).then (result => {
            ...
            logger.error( 'Error submitting block at height %d from %s@%s, share type: "%s" - %j', [job.height, miner.login, miner.ip, shareType, error]);
            recordShareData(miner, job, hashDiff.toString(), false, null, shareType);
        });
    } else if (hashDiff.lt(job.difficulty)){
        logger.warn( 'Rejected low difficulty share of %s from %s@%s', [hashDiff.toString(), miner.login, miner.ip]);
        return false;
    } else {
        recordShareData(miner, job, hashDiff.toString(), false, null, shareType);
    }
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch(method){
        case 'login':
            ...
            logger.info( 'Miner connected %s@%s',  [params.login, miner.ip]);
            break;
        case 'getjob':
            if (!miner){
        ...
        case 'submit':
            ...
            if (!noncePattern.test(params.nonce)) {
                ...
                logger.warn( 'Malformed nonce: ' + JSON.stringify(params) + ' from ' + minerText);
            ...
            if (job.submissions.indexOf(params.nonce) !== -1){
                 var minerText = miner ? (' ' + miner.login + '@' + miner.ip) : '';
                 logger.warn( 'Duplicate share: ' + JSON.stringify(params) + ' from ' + minerText);
                 perIPStats[miner.ip] = { validShares: 0, invalidShares: 999999 };
                 miner.checkBan(false);
                 sendReply('Duplicate share');
                 return;
            ...
            if (shareTrustEnabled){
                if (shareAccepted){
                    ...
                    logger.warn( 'Share trust broken by %s@%s', [miner.login, miner.ip]);
        ...
        default:
            ...
            var minerText = miner ? (' ' + miner.login + '@' + miner.ip) : '';
            logger.warn( 'Invalid method: %s (%j) from %s', [method, params, minerText]);
            break;
    }
}
    ...
    return Promise.each(config.poolServer.ports, portData => {
        return new Promise( (resolve, reject) => {
            var handleMessage = function(socket, jsonData, pushMessage){
                if (!jsonData.id) {
                    logger.warn( 'Miner RPC request missing RPC id');
                    return;
                }
                else if (!jsonData.method) {
                    logger.warn( 'Miner RPC request missing RPC method');
                    return;
                }
                else if (!jsonData.params) {
                    logger.warn( 'Miner RPC request missing RPC params');
                    return;
                }
            ...
            net.createServer(function(socket){
                ...
                socket.on('data', function(d){
                    ...
                    if (Buffer.byteLength(dataBuffer, 'utf8') > 10240){ //10KB
                        dataBuffer = null;
                        logger.warn( 'Socket flooding detected and prevented from %s', [socket.remoteAddress]);
                        socket.destroy();
                        return;
                    }
                    if (dataBuffer.indexOf('\n') !== -1){
                        ...
                        for (var i = 0; i < messages.length; i++){
                            ...
                            try{
                                ...
                                logger.warn( 'Malformed message from %s: %s', [socket.remoteAddress, message]);
                                socket.destroy();
                    ...
                    if (err.code !== 'ECONNRESET')
                        logger.warn( 'Socket error from %s %j', [socket.remoteAddress, err]);
                }).on('close', function(){
                ...
                if (error) {
                    logger.error( 'Could not start server listening on port %d, error: $j', [portData.port, error]);
                    return reject(error);
                }
                logger.info( 'Started server listening on port %d', [portData.port]);
                return resolve(true);
            });
        });
    });
