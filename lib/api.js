...
function broadcastLiveStats(){
    ...
    logger.info('Broadcasting to %d visitors and %d address lookups', [Object.keys(liveConnections).length, Object.keys(addressConnections).length]);
...
function handleMinerStats(urlParts, response){
    ...
    if (urlParts.query.longpoll === 'true') {
        redisClient.exists(config.coin + ':workers:' + address).then( result => {
            if (!result) {
        ...
        ]).exec().then(redisClient.convertMultiReply).then( replies => {
            if (!Object.keys(replies[0]).length) {
                throw new Error('Not found');
            }
            var stats = replies[0];
            stats.hashrate = minerStats[address];
...
server.listen(config.api.port, function(){
    logger.info('API started & listening on port %d', [config.api.port]);
});